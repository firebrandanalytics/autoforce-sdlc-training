...
- Then work at the **repo root** in a Claude Code session (so the agent can read
  both `data/vol_report.py` and `data/autoforce.sqlite`).
...
> each value in `data/autoforce.sqlite` from the rows first, then check `code_ref`
...
> equivalent figure from the `rin_transactions` table in `data/autoforce.sqlite`.
