...
Autoforce AI Training · the general task-spec template. For a **data deliverable**, pair this with **D4 (Data Spec Template)**.
...
>
> **Context:** Data is in `data/autoforce.sqlite`, table `lifts` (see the schema with `.schema lifts`). We report on `net_gal` (temperature-corrected). Voided rows have `status = 8` and should be excluded from this check — a void isn't "suspicious," it's already handled.
>
