...
Autoforce AI Training · Session 3 (Exercise 1) reference card

How to read `EXPLAIN QUERY PLAN`, recognise a slow full scan, and decide when an index actually helps. Every example here runs against `data/autoforce.sqlite` and produces the output shown.
...
```
Run EXPLAIN QUERY PLAN on this query against data/autoforce.sqlite and
