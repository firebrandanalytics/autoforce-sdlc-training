# `data/` — the Autoforce course dataset
...
| File | What it is |
|---|---|
| `autoforce.sqlite` | The database (~50k rows in the `lifts` fact table). This is the file you query. |
...
con = sqlite3.connect("autoforce.sqlite")
...
> that lets it find `autoforce.sqlite` — usually this `data/` folder, or wherever
...
**Open Database**, and point it at `autoforce.sqlite`. The **Browse Data** tab
