...
Autoforce AI Training · Session 3 · Hands-on (~60 min)
...
The dataset is at `data/autoforce.sqlite` from the repo root. No installs beyond
`sqlite3` (and Python, which you have). Confirm it opens:

```bash
# from the repo root
sqlite3 data/autoforce.sqlite ".tables"
```
...
> Read this query (paste it). Against `data/autoforce.sqlite`, describe in plain
...
> `data/autoforce.sqlite` — the fact tables `lifts`, `rin_transactions`,
...
> Run `EXPLAIN QUERY PLAN` on the query against `data/autoforce.sqlite` and
...
> comment claims, and (c) which one the data in `data/autoforce.sqlite` supports —
