...
Autoforce AI Software-Development Training · Session 4 (120 min)
...
```
<some folder>/
  autoforce-sdlc-training/        ← the course repo (READ-ONLY reference today)
...
cd ..
ls
#    ^ you should SEE "autoforce-sdlc-training" in that listing. If you don't,
...
```bash
python3 -c "import sqlite3; print(sqlite3.connect('../autoforce-sdlc-training/data/autoforce.sqlite').execute('SELECT COUNT(*) FROM lifts').fetchone())"
# -> (50025,)
( cd ../autoforce-sdlc-training/data && python3 vol_report.py 2025-08 )
...
> **`../autoforce-sdlc-training/data/`** (the database, the dictionary,
> `vol_report.py`) — never `../autoforce-sdlc-training/sessions/session-5/`
...
```
Here is my Homework 2 intent dossier for the legacy script (it lives in the course repo at ../autoforce-sdlc-training/data/vol_report.py):
...
```
Create a skill at .claude/skills/autoforce-volume-rules/SKILL.md capturing the
verified rules of this dataset from my Homework 2 dossier, pasted below. Format:
YAML frontmatter with name: autoforce-volume-rules and a one-line description
...
> Read only `.claude/skills/autoforce-volume-rules/SKILL.md`. Summarize the
...
where work lives at Autoforce. Then everything so far lands on `main` as the
...
autoforce-volume-rules skill. The plan must include:
...
1. TESTS FIRST: test_service.py exists and runs (failing) before service.py.
   ...
   "cd ../autoforce-sdlc-training/data && python3 vol_report.py 2025-08", not from memory.
...
3b. The DB path is CONFIGURABLE, not hard-coded: read it from a DB_PATH
   ...
   ../autoforce-sdlc-training/data/autoforce.sqlite (the course repo sits next
